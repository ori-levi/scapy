...
def do_graph(graph, prog=None, format=None, target=None, type=None, string=None,
             options=None):
    ...
    if format is not None:
        format = "-T %s" % format
    w, r = os.popen2(
        "%s %s %s %s" % (prog, options or "", format or "", target))
    w.write(graph)
    w.close()
    if start_viewer:
            ...
            for j in xrange(len(lstcol)):
                for k in xrange(len(lstcol)):
                    if i != j or j != k or k != i:
                        yield trans(lstcol[(i + j) % len(lstcol)],
...
class EnumElement:
    ...
    def __repr__(self):
        return "<%s %s[%r]>" % (
        self.__dict__.get("_name", self.__class__.__name__), self._key,
        self._value)
...
sync: do not bufferize writes to the capture file
        ...
        if isinstance(filename, basestring):
            self.filename = filename
            self.f = [open, gzip.open][gz](filename, append and "ab" or "wb",
                                           gz and 9 or bufsz)
        else:
            self.f = filename
...
def make_table(*args, **kargs):
    __make_table(lambda l: "%%-%is" % l, lambda l: "%%-%is" % l, "", *args,
                 **kargs)
