...
from scapy.ansmachine import AnsweringMachine, conf
from scapy.utils import warning, inet_aton, inet_ntop, inet_pton
from scapy.fields import StrField, ShortField, StrLenField, Field, BitField, \
    BitEnumField, ShortEnumField, FieldLenField, ByteField, PacketListField, \
    IntField, ByteEnumField, FlagsField
...
class DNSRRCountField(ShortField):
    ...
    def _count_rr(self, pkt):
        x = getattr(pkt, self.rr)
        i = 0
        while isinstance(x, DNSRR) or isinstance(x, DNSQR) or isdnssec_rr(x):
            x = x.payload
            i += 1
        return i
...
class DNSRRField(StrField):
    ...
    def decode_rr(self, name, s, p):
        ret = s[p:p + 10]
        type_, cls, ttl, rdlen = struct.unpack("!HHIH", ret)
        p += 10
        rr = DNSRR("\x00" + ret + s[p:p + rdlen])
        if type_ in [2, 3, 4, 5]:
            rr.rdata = dns_getstr(s, p)[0]
            del rr.rdlen
        elif type_ in DNSRR_DISPATCHER:
            rr = DNSRR_DISPATCHER[type_]("\x00" + ret + s[p:p + rdlen])
        else:
            del rr.rdlen
    ...
    def getfield(self, pkt, s):
        ...
        while c:
            c -= 1
            name, p = dns_getstr(s, p)
            rr, p = self.decode_rr(name, s, p)
            if ret is None:
                ret = rr
            else:
                ret.add_payload(rr)
        ...
        if pkt.type == 1:  # A
            ...
            while tmp_s:
                ...
                if tmp_len > len(tmp_s):
                    warning(("DNS RR TXT prematured end of character-string "
...
class EDNS0TLV(Packet):
    name = "DNS EDNS0 TLV"
    fields_desc = [ShortEnumField("optcode", 0,
                                  {0: "Reserved", 1: "LLQ", 2: "UL", 3: "NSID",
                                   4: "Reserved", 5: "PING"}),
...
dnssecalgotypes = {0: "Reserved", 1: "RSA/MD5", 2: "Diffie-Hellman",
                   3: "DSA/SHA-1",
...
dnssecdigesttypes = {0: "Reserved", 1: "SHA-1", 2: "SHA-256",
...
def rr_list_to_bitmap(lst):
    ...
    for wb in xrange(min_window_blocks, max_window_blocks + 1):
        ...
        if rrlist[-1] == 0:  # only one element in the list
            ...
            max_ = rrlist[-1] - 256 * wb
            bytes_ = int(math.ceil(max_ / 8)) + 1  # use at least 1 byte
        if bytes_ > 32:  # Don't encode more than 256 bits / values
            bytes_ = 32

        bitmap += struct.pack("B", wb)
        bitmap += struct.pack("B", bytes_)

        # Generate the bitmap
        for tmp in xrange(bytes_):
            v = 0
            # Remove out of range Ressource Records
            tmp_rrlist = filter(
                lambda x: 256 * wb + 8 * tmp <= x < 256 * wb + 8 * tmp + 8,
                rrlist)
            if not tmp_rrlist == []:
...
class RRlistField(StrField):
    def h2i(self, pkt, x):
        if type(x) == list:
            return rr_list_to_bitmap(x)
        return x

    def i2repr(self, pkt, x):
        x = self.i2h(pkt, x)
        rrlist = bitmap_to_rr_list(x)
...
class DNSRRNSEC3PARAM(_DNSRRdummy):
    ...
    fields_desc = [DNSStrField("rrname", ""),
                   ...
                   FieldLenField("saltlength", 0, fmt="!B", length_of="salt"),
                   StrLenField("salt", "",
                               length_from=lambda pkt: pkt.saltlength)
                   ]
...
def dyndns_add(nameserver, name, rdata, type_="A", ttl=10):
...
@conf.commands.register
def dyndns_del(nameserver, name, type_="ALL", ttl=10):
...
"""
    ...
    r = sr1(IP(dst=nameserver) / UDP() / DNS(opcode=5,
                                             qd=[DNSQR(qname=zone,
                                                       qtype="SOA")],
                                             ns=[DNSRR(rrname=name, type=type_,
                                                       rclass="ANY", ttl=0,
                                                       rdata="")]),
            verbose=0, timeout=5)
    if r and r.haslayer(DNS):
...
class DNSam(AnsweringMachine):
    function_name = "dns_spoof"
    filter = "udp port 53"

    def __init__(self, **kwargs):
        super(DNSam, self).__init__(**kwargs)
        self.match = None
        self.joker = None
