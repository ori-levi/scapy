...
# ### ASN1 ####
###############
class ASN1Error(Scapy_Exception):
    pass
...
class ASN1BadTagDecodingError(ASN1DecodingError):
    pass


class ASN1Codec(EnumElement):
    def __init__(self, key, value):
        super(ASN1Codec, self).__init__(key, value)
        self._stem = None

    def register_stem(self, stem):
        self._stem = stem

    def dec(self, s, context=None):
        return self._stem.dec(s, context=context)

    def safedec(self, s, context=None):
        return self._stem.safedec(s, context=context)

    def get_stem(self):
        return self.stem


class ASN1CodecsMetaclass(Enum_metaclass):
    element_class = ASN1Codec


class ASN1Codecs:
    __metaclass__ = ASN1CodecsMetaclass
    BER = 1
    DER = 2
    PER = 3
    CER = 4
...
class ASN1Tag(EnumElement):
    ...
    def clone(self):  # /!\ not a real deep copy. self.codec is shared
        return self.__class__(self._key, self._value, self._context,
                              self._codec)
    ...
    def asn1_object(self, val):
        ...
        raise ASN1Error("%r does not have any assigned ASN1 object" % self)
    ...
    def get_codec(self, codec):
        try:
            ...
            raise ASN1Error("Codec %r not found for tag %r" % (codec, self))
        return c


class ASN1ClassMetaclass(Enum_metaclass):
    element_class = ASN1Tag

    def __new__(cls, name, bases,
                dct):  # XXX factorise a bit with Enum_metaclass.__new__()
        for b in bases:
...
class ASN1_Class:
    __metaclass__ = ASN1ClassMetaclass
...
class ASN1_Object:
    ...
    def __repr__(self):
        return "<%s[%r]>" % (
        self.__dict__.get("name", self.__class__.__name__), self.val)
...
class ASN1_INTEGER(ASN1_Object):
    ...
    def __repr__(self):
        ...
        if len(r) > 20:
            r = r[:10] + "..." + r[-10:]
        return h + " <%s[%s]>" % (
        self.__dict__.get("name", self.__class__.__name__), r)
...
class ASN1_BIT_STRING(ASN1_Object):
    ...
    def __repr__(self):
        if len(self.val) <= 16:
            ...
            self.__dict__.get("name", self.__class__.__name__), self.val,
            self.unused_bits, "s" if self.unused_bits > 1 else "")
        else:
            s = self.val_readable
            if len(s) > 20:
                s = s[:10] + "..." + s[-10:]
            return "<%s[%r] (%d unused bit%s)>" % (
            self.__dict__.get("name", self.__class__.__name__), s,
            self.unused_bits, "s" if self.unused_bits > 1 else "")
...
class ASN1_OID(ASN1_Object):
    ...
    def __repr__(self):
        return "<%s[%r]>" % (
        self.__dict__.get("name", self.__class__.__name__), self.oidname)
...
class ASN1_GENERALIZED_TIME(ASN1_STRING):
    ...
    def __setattr__(self, name, value):
        if name == "val":
            ...
            super(ASN1_GENERALIZED_TIME, self).__setattr__("pretty_time",
                                                           pretty_time)
...
conf.ASN1_default_codec = ASN1Codecs.BER
